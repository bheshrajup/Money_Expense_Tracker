<?php  
  class Expense extends Base {
    ...
    // Example implementation in your Expense class (Expense.php)
public function updateExpense($expense_id, $new_item, $new_cost, $new_date) {
  // Example SQL query to update expense
  $stmt = $this->pdo->prepare("UPDATE expense SET Item = :item, Cost = :cost, Date = :date WHERE ID = :id");
  $stmt->bindParam(":item", $new_item);
  $stmt->bindParam(":cost", $new_cost);
  ...
  public function getExpenseById($expense_id) {
    try {
        $stmt = $this->pdo->prepare("SELECT * FROM expense WHERE ID = :expense_id");
        $stmt->bindParam(":expense_id", $expense_id, PDO::PARAM_INT);
        $stmt->execute();
        $expense = $stmt->fetch(PDO::FETCH_OBJ);
        return $expense; // Return the fetched expense object
